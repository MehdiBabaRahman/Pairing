...
// declaring the branches as floats 
  ...
  Float_t diMuonC_FittedVtx_pt;
  Float_t diMuonF_FittedVtx_pt;
...
TString saveMW1 = "Figures/Diff_MassW_200_";
...
saveMW = saveMW1 + masses[ii] + pdf;
  ...
  // TCanvas *cnv2 = new TCanvas();
    ...
    // TPad *pad1 = new TPad("pad1","",0,0,1,1);
     ...
     TH1F *h2 = new TH1F(title,"",100,-30,30);

     TH1F *h4 = new TH1F(title,"",100,-30,30);

     TH1F *h3 = new TH1F(title,"",100,30,30);

     TH1F *h5 = new TH1F(title,"",100,-30,30);
     ...
     // h2->SetMarkerStyle(7);
     // h2->SetMarkerSize(4);
     h2->SetXTitle("#DeltaM [GeV]");
     h2->SetYTitle("Number of Events");
     h4->SetXTitle("#DeltaM [GeV]");
     h4->SetYTitle("Number of Events");
     h3->SetXTitle("#DeltaM [GeV]");
     h3->SetYTitle("Number of Events");
     h5->SetXTitle("#DeltaM [GeV]");
     h5->SetYTitle("Number of Events");
...
// declaring branch addresses 
  ...
  myTree1->SetBranchAddress("genDiMu0_Pt",&genDiMu0_Pt);
  myTree1->SetBranchAddress("genDiMu1_Pt",&genDiMu1_Pt);

  myTree1->SetBranchAddress("diMuonC_FittedVtx_px",&diMuonC_FittedVtx_px);
  myTree1->SetBranchAddress("diMuonC_FittedVtx_py",&diMuonC_FittedVtx_py);

  myTree1->SetBranchAddress("diMuonF_FittedVtx_px",&diMuonF_FittedVtx_px);
  myTree1->SetBranchAddress("diMuonF_FittedVtx_py",&diMuonF_FittedVtx_py);  
...
for(int ii = 0; ii < N; ii++){
    myTree1->GetEntry(ii);
        ...
        diMuonF_FittedVtx_pt = sqrt( (diMuonF_FittedVtx_px *  diMuonF_FittedVtx_px) + (diMuonF_FittedVtx_py * diMuonF_FittedVtx_py) );
        diMuonC_FittedVtx_pt = sqrt( (diMuonC_FittedVtx_px *  diMuonC_FittedVtx_px) + (diMuonC_FittedVtx_py * diMuonC_FittedVtx_py) );
   	...
   	if ( ( genA0_Lxy < 16.0 && fabs(genA0_Lz) < 51.6 ) && ( genA1_Lxy < 16.0 && fabs(genA1_Lz) < 51.6 ) ){
   		if ( is1SelMuHighPt== true && is2SelMuHighPt== true && is3SelMuLowPt== true && is4SelMuLowPt== true){
               ...
               diMuonF_FittedVtx_prob > P0*(1 - dimuF_nSAMu)*exp( -( p0 + p1*(sqrt(diMuonF_FittedVtx_dR)) + p2*pow(sqrt(diMuonF_FittedVtx_dR), 2) + p3*pow(sqrt(diMuonF_FittedVtx_dR), 3) + p4*pow(sqrt(diMuonF_FittedVtx_dR), 4) )*pow(fabs(diMuonF_FittedVtx_Lxy/R0), C0) ) ) {
                      ...
                      if (diMuonF_FittedVtx_pt < diMuonC_FittedVtx_pt){

   										h2->Fill(genDiMu1_M -  massF);
                      h3->Fill(genDiMu0_M -  massC );
                    }

                      else {
                           h4->Fill( genDiMu1_M -  massC );
                           h5->Fill( genDiMu0_M -  massF);
                      }
  ...
  TLegend* legend = new TLegend(0.7,0.67,0.894,0.77);
  legend->AddEntry( h2, "Trailing", "l");
  legend->AddEntry( h3, "Leading", "l");
