...
TString savePT1 = "Figures/sel_pt_200_";

TString saveEta1 = "Figures/sel_eta_200_";
...
saveEta = saveEta1 + masses[ii] + pdf;
   ...
   TFile *myFile = new TFile(temp);
    ...
    TH1F *h1 = new TH1F(title, "sel #mu p_{T}", 100, 0, 150);
    h1->SetLineColor(1);
    h1->GetYaxis()->SetRangeUser(0,250);

    //TCanvas *cnv2 = new TCanvas();
    TH1F *h2 = new TH1F("Subleading sel di#mu p_{T} ", "Sub-leading sel di#mu p_{T}", 100, 0, 150);
    h2->SetLineColor(2);
    h2->GetYaxis()->SetRangeUser(0,250);

   ///TCanvas *cnv3 = new TCanvas();
    TH1F *h3 = new TH1F("3^{rd} sel di#mu p_{T}", "3^{rd} sel di#mu p_{T}", 100, 0, 150);
    h3->SetLineColor(3);
    h3->GetYaxis()->SetRangeUser(0,250);

    //TCanvas *cnv4 = new TCanvas();
    TH1F *h4 = new TH1F("4^{th} sel di#mu p_{T}", "4^{th} sel di#mu p_{T}", 100, 0, 150);
    h4->SetLineColor(4);
    h4->GetYaxis()->SetRangeUser(0,250);


    TCanvas *cnv5 = new TCanvas();
    TH1F *h5 = new TH1F(title, "sel #mu #eta", 100, -5, 5);
    h5->SetLineColor(1);
    h5->GetYaxis()->SetRangeUser(0,80);

    //TCanvas *cnv6 = new TCanvas();
    TH1F *h6 = new TH1F("Subleading sel di#mu #eta", "Sub-leading sel di#mu #eta", 100, -5, 5);
    h6->SetLineColor(2);
    h6->GetYaxis()->SetRangeUser(0,80);

   // TCanvas *cnv7 = new TCanvas();
    TH1F *h7 = new TH1F("3^{rd} sel di#mu #eta", "3^{rd} sel di#mu #eta", 100, -5, 5);
    h7->SetLineColor(3);
    h7->GetYaxis()->SetRangeUser(0,80);

  //  TCanvas *cnv8 = new TCanvas();
    TH1F *h8 = new TH1F("4^{th} sel di#mu #eta", "4^{th} sel di#mu #eta", 100, -5, 5);
    h8->SetLineColor(4);
